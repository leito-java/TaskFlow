...
export class ProjectsPageComponent {
  private readonly api=inject(ProjectApiService); protected readonly projects=signal<Project[]>([]); protected readonly error=signal(''); protected name='';
  constructor(){ this.load(); } load(){this.api.getProjects().subscribe({next:p=>this.projects.set(p),error:()=>this.error.set('Impossible de charger les projets.')});} create(){this.api.createProject(this.name.trim()).subscribe({next:p=>{this.projects.update(x=>[...x,p]);this.name='';},error:()=>this.error.set('Impossible de créer ce projet.')});}
}
